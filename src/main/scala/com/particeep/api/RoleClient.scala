...
class RoleClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS with WithCredentials with EntityClient {
  ...
  def all(timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[String]]] = {
    ws.get[List[String]](s"$endPoint/all", timeout)
  }

  def allByUser(user_id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Roles]] = {
    ws.get[Roles](s"$endPoint/all_by_user/$user_id", timeout)
  }

  def add(user_id: String, role: String, role_creation: RoleCreation, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Roles]] = {
    ws.put[Roles](s"$endPoint/$user_id/add/${role.toLowerCase}", timeout, Json.toJson(role_creation))
  }

  def addRoles(roles_creation: List[RolesCreation], timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[Roles]]] = {
  ...
  def hasRole(user_id: String, role: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Boolean]] = {
    allByUser(user_id).map(result => result.right.map(roles => roles.roles.map(_.role_name).contains(role)))
  }

  def search(criteria: RoleSearch, table_criteria: TableSearch, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, PaginatedSequence[Roles]]] = {
