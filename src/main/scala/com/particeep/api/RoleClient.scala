...
import com.particeep.api.models.role.Roles
...
class RoleClient(ws: WSClient) extends ResponseParser {
  ...
  def add(user_id: String, role: String, timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Roles]] = {
    ws.url(s"$endPoint/$user_id/add/${role.toLowerCase}", timeout).post(Results.EmptyContent()).map(parse[Roles])
  }
