...
class EnterpriseClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS with WithCredentials with EntityClient {
  ...
  def byId(id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Enterprise]] = {
    ws.get[Enterprise](s"$endPoint/$id", timeout)
  }

  def byIds(ids: List[String], timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[Enterprise]]] = {
    ws.get[List[Enterprise]](s"$endPoint", timeout, List("ids" -> ids.mkString(",")))
  }

  def byUserId(user_id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[Enterprise]]] = {
    ws.get[List[Enterprise]](s"$endPoint/user/$user_id", timeout)
  }

  def create(enterprise_creation: EnterpriseCreation, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Enterprise]] = {
    ws.put[Enterprise](s"$endPoint", timeout, Json.toJson(enterprise_creation))
  }

  def update(id: String, enterprise_edition: EnterpriseEdition, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Enterprise]] = {
    ws.post[Enterprise](s"$endPoint/$id", timeout, Json.toJson(enterprise_edition))
  }

  def search(criteria: EnterpriseSearch, table_criteria: TableSearch, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, PaginatedSequence[Enterprise]]] = {
  ...
  def delete(id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Enterprise]] = {
    ws.delete[Enterprise](s"$endPoint/$id", timeout)
  }

  def addManager(id: String, manager_creation: ManagerCreation, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, ManagerLink]] = {
    ws.put[ManagerLink](s"$endPoint/$id/manager", timeout, Json.toJson(manager_creation))
  }

  def getManagers(id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[ManagerLink]]] = {
    ws.get[List[ManagerLink]](s"$endPoint/$id/manager", timeout)
  }

  def deleteManager(id: String, manager_id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, ManagerLink]] = {
    ws.delete[ManagerLink](s"$endPoint/$id/manager/$manager_id", timeout)
  }

  def nbEnterprisesByActivityDomain(timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Seq[NbEnterprisesByActivityDomain]]] = {
    ws.get[Seq[NbEnterprisesByActivityDomain]](s"$endPoint/info/activity/domain", timeout)
  }

  def importFromCsv(csv: MultipartFormData[TemporaryFile], timeout: Long = defaultImportTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, ImportResult[Enterprise]]] = {
