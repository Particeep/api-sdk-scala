...
class NewsClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS with WithCredentials with EntityClient {
  ...
  def byIds(ids: Seq[String], timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[News]]] = {
    ws.get[List[News]](s"$endPoint", timeout, List("ids" -> ids.mkString(",")))
  }

  def getPrevAndNext(id: String, limit: Int, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, NewsPrevAndNext]] = {
    ws.get[NewsPrevAndNext](s"$endPoint/with-prev-next/$id/$limit", timeout)
  }

  def delete(id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, News]] = {
    ws.delete[News](s"$endPoint/$id", timeout)
  }

  def add(news_creation: NewsCreation, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, News]] = {
    ws.put[News](s"$endPoint", timeout, Json.toJson(news_creation))
  }

  def edit(id: String, news_edition: NewsEdition, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, News]] = {
    ws.post[News](s"$endPoint/$id", timeout, Json.toJson(news_edition))
  }

  def search(criteria: NewsSearch, table_criteria: TableSearch, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, PaginatedSequence[News]]] = {
