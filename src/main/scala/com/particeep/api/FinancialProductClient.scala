...
class FinancialProductClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS with WithCredentials with EntityClient {
  ...
  def byId(id: String, timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, FinancialProduct]] = {
    ws.get[FinancialProduct](s"$endPoint/$id", timeout)
  }

  def create(financial_product_creation: FinancialProductCreation, timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, FinancialProduct]] = {
    ws.put[FinancialProduct](s"$endPoint", timeout, Json.toJson(financial_product_creation))
  }

  def update(id: String, financial_product_edition: FinancialProductEdition, timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, FinancialProduct]] = {
    ws.post[FinancialProduct](s"$endPoint/$id", timeout, Json.toJson(financial_product_edition))
  }

  def search(criteria: FinancialProductSearch, table_criteria: TableSearch, timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, PaginatedSequence[FinancialProduct]]] = {
  ...
  def delete(id: String, timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, FinancialProduct]] = {
    ws.delete[FinancialProduct](s"$endPoint/$id", timeout)
  }

  def getComposition(id: String, timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[FinancialProduct]]] = {
  ...
  def removeFromComposition(id: String, ids: Seq[String], timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[FinancialProduct]]] = {
