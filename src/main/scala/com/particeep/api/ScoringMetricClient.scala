...
class ScoringMetricClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS with WithCredentials with EntityClient {
  ...
  def searchMetrics(timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Seq[ScoringMetric]]] = {
    ws.get[Seq[ScoringMetric]](s"$endPoint/search/", timeout)
  }

  def metricsById(metric_id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, ScoringMetric]] = {
  ...
  def evaluationsById(eval_id: String, metric_id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, ScoringEvaluation]] = {
