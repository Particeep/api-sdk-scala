...
class WalletClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS with WithCredentials with EntityClient {
  ...
  def byId(id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Wallet]] = {
    ws.get[Wallet](s"$endPoint/$id", timeout)
  }

  def byTargetIdAndType(owner_id: String, owner_type: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Wallet]] = {
    ws.get[Wallet](s"$endPoint/owner/$owner_id/$owner_type", timeout)
  }

  def consumerWallet(timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Wallet]] = {
    ws.get[Wallet](s"$endPoint/consumer", timeout)
  }

  def create(wallet_creation: WalletCreation, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Wallet]] = {
    ws.put[Wallet](s"$endPoint", timeout, Json.toJson(wallet_creation))
  }

  def cashin(id: String, cash_in: CashIn, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, String]] = {
    ws.post[String](s"$endPoint/$id/cashin", timeout, Json.toJson(cash_in))
  }

  def withdraw(id: String, cash_out: CashOut, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, TransactionWallet]] = {
    ws.post[TransactionWallet](s"$endPoint/$id/cashout", timeout, Json.toJson(cash_out))
  }

  def transfer(transfer: WalletTransfer, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, TransactionWallet]] = {
    ws.post[TransactionWallet](s"$endPoint/transfer", timeout, Json.toJson(transfer))
  }

  def allRelatedTransactions(id: String, criteria: TableSearch, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, PaginatedSequence[TransactionWallet]]] = {
  ...
  def addBankAccount(id: String, bank_account_creation: BankAccountCreation, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, BankAccount]] = {
    ws.put[BankAccount](s"$endPoint/$id/bankaccount", timeout, Json.toJson(bank_account_creation))
  }

  def getBankAccountsByWalletId(id: String, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Seq[BankAccount]]] = {
    ws.get[Seq[BankAccount]](s"$endPoint/$id/bankaccount", timeout)
  }

  def cashinBankAccount(id: String, cash_in_bank_account_creation: CashInBankAccountCreation, timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, CashInBankAccount]] = {
