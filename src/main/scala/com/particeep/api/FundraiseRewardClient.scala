...
class FundraiseRewardClient(ws: WSClient) extends ResponseParser {
  ...
  def buyReward(fundraise_id: String, reward_id: String, user_id: String, donation: Amount, timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Transaction]] = {
