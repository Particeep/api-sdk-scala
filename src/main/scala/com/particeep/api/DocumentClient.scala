...
class DocumentClient(ws: WSClient) extends ResponseParser {
  ...
  def upload(owner_id: String, file: MultipartFormData[TemporaryFile], document_creation: DocumentCreation, timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, Document]] = {
    val url = s"$endPoint/$owner_id/upload"
    Future { prepareRequest(url, file, document_creation, timeout).get() }.map(parse[Document])
  }

  private[this] def prepareRequest(url: String, file: MultipartFormData[TemporaryFile], document_creation: DocumentCreation, timeout: Long)(implicit exec: ExecutionContext): ListenableFuture[Response] = {
  ...
  def listDir(path: String, target_id: Option[String], target_type: Option[String], timeout: Long = -1)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[FolderOrFile]]] = {
