...
class FundraiseSearchClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS with WithCredentials with EntityClient {
  ...
  def byIds(ids: List[String], timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[FundraiseData]]] = {
    ws.get[List[FundraiseData]](s"$endPoint/byIds", timeout, List("ids" -> ids.mkString(",")))
  }

  def nbProjectsByActivityDomain(categories: List[String], timeout: Long = defaultTimeOutInSeconds)(implicit exec: ExecutionContext): Future[Either[ErrorResult, List[NbProjectsByCategory]]] = {
